...
public class StringUtil {
	...
	public final static String PHONE_DELIMITER = "[,;、，；.。:：'\r\n\\s]\\s*";
	...
	public static Timestamp stringToTimestamp(String strTime, Timestamp defaultValue) {
		...
		try {
			if (strTime != null && strTime.length() > 0) time = java.sql.Timestamp.valueOf(strTime);
		} catch (IllegalArgumentException e) {
		}
		return time;
	}
	...
	public static String formatCommaString(String value) {
		...
		String tmp[] = value.trim().split(PHONE_DELIMITER);
