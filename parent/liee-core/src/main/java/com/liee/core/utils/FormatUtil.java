...
public class FormatUtil {
	...
	public static StringBuilder formatBinary(byte binary,
		...
		return formatBinary(binary, toAppendTo, EncryUtil.EIGHT_BIT);
	}
	...
	//1,5,"0"
	public static String formatNumber(int sourceNum, int scale, String fills){
    	StringBuffer sb = new StringBuffer();
    	String souNum = String.valueOf(sourceNum);
    	for(int i = souNum.length(); i < scale; i++){
    		sb.append(fills);
    	}
    	sb.append(souNum.length()>scale?souNum.substring(0, scale):souNum);
    	return sb.toString();
    }
